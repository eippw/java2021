...
public class WorldHappinessJake implements DataSet {
    ...
    public double getCountryMeanLadder(String countryName) {
        int c = getCount(countryName);
        if (c == 0)
            return -1;
        int idx = getStartIndex(countryName);
        double sum = 0;
        for (int row = idx; row < (idx + c); row++) {
   ...
   /**
    ...
    // Method for happiest and least happy country based on mean ladder score..... Moved to here to clean up tester
    public void happyNahHappy() {
        double topMean = 0;
        double lowMean = 999;
        String topCountry = "";
        String lowCountry = "";
        ...
        for (int i = 0; i < c.length; i++) {
            double mean = getCountryMeanLadder(c[i]);
            if (mean > topMean) {
                topMean = mean;
                topCountry = c[i];
            }
        ...
        System.out.println("The happiest country is: " + topCountry + " and the least happy country is: " + lowCountry);
    }
}
