...
public class WorldHappinessIvy implements DataSet {
    ...
    public WorldHappinessIvy(String infn, String outfn) throws IOException {
        this.infn = infn;
        this.outfn = outfn;
        initData();
    }
    ...
    public double getCountryMeanLadder(String countryName) {
        int count = 0;
        double ladderScore = 0.0;   
            for(int row = 0; row < tdata.length; row++){
                for(int col = 0; col < tdata[0].length; col++){
                    if (countryName.equalsIgnoreCase(tdata[row][col])){
                       double score = ndata[row][1];
                       ladderScore += score;
                       //System.out.println(row + "\tLadder Score: " + score);
                        count++;
    ...
    public int getStartIndex(String countryName) {
     ...
     /**
     * Return the mean value for the Healthy Life Expectance at Birth data for a particular country
     * @param countryName The name of the country to calculate the mean life expectancy of
     * @reutun the average of the life expectancy scores for a country or -1 if the country count is 0
     */
    public double getCountryMeanLifeExpectancy(String countryName) {
         int count = 0;
        double lifeExp = 0.0;   
            for(int row = 0; row < tdata.length; row++){
                for(int col = 0; col < tdata[0].length; col++){
                    if (countryName.equalsIgnoreCase(tdata[row][col])){
                       double score = ndata[row][5];
                       lifeExp += score;
                       //System.out.println(row + "\tLife Expectance: " + score);
                        count++;
   ...
   /**
     * Return the mean value for the Perceptions of Corruptions data for a particular country
     * @param countryName The name of the country to calculate the mean perception of corruption
     * @reutun the average of the life expectancy scores for a country or -1 if the country count is 0
     */
    public double getCountryMeanCorruptionPerception(String countryName){
        int count = 0;
        double poc = 0.0;   
            for(int row = 0; row < tdata.length; row++){
                for(int col = 0; col < tdata[0].length; col++){
                    if (countryName.equalsIgnoreCase(tdata[row][col])){
                       double score = ndata[row][8];
                       poc += score;
                       //System.out.println(row + "\tPerceptions of Corruptions: " + score);
                        count++;
