...
public class WorldHappinessCam2 implements DataSet {
    ...
    public WorldHappinessCam2(String infn, String outfn) throws IOException {
        this.infn = infn;
        this.outfn = outfn;
        initData();
    }
    ...
    public double getCountryMeanLadder(String countryName) {
        ...
        for(int i = 0; i < tdata.length; i++){
            if (tdata[i][0].equalsIgnoreCase(countryName)){
                double score = ndata[i][1];
                //System.out.println(i + "\tLadder Score: " + score);
                ave += score;
            }
        }
        
        
        double average = ave/count;
        return average;
    }
    ...
    public double getCountryMeanLifeExpectancy(String countryName){
        ...
        for (int i = 0; i < ndata.length; i++){
            if (tdata[i][0].equalsIgnoreCase(countryName)){
                double meanLifeScore = ndata[i][4];
                avg += meanLifeScore;
            }
        }
        double avgMeanLifeScore = avg / count;
        return avgMeanLifeScore;
    } 
   ...
   /**
    ...
    public double getCountryMeanCorruptionPerception(String countryName){
        ...
        for (int i = 0; i < ndata.length; i++){
            if (tdata[i][0].equalsIgnoreCase(countryName)){
                double corrupt = ndata[i][7];
                avg += corrupt;
            }
        }
        double avgCorrupt = avg / count;
        return avgCorrupt;
    }
